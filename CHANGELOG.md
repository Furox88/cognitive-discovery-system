...
### Added
- **`cds[all]` meta-extras**: `pip install cognitive-discovery-platform[all]` installs every optional group (dashboard, test, dev, docs) in one command. Saves typing and ensures a full dev experience.
- **`scripts/publish.py --skip-docs` flag** (off by default): controls whether the publish step also runs `mkdocs gh-deploy` to update GitHub Pages.

### Changed
- **Removed redundant `docs/api-reference.md`** (227 lines of hand-written static table) in favor of `docs/api.md`, which is auto-generated by mkdocstrings from the source. `mkdocs.yml` nav now points only to the auto-generated page.
- **`scripts/publish.py`** now deploys docs to GitHub Pages on a successful PyPI upload (legacy `gh-pages` branch source).
