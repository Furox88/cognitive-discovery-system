...
Provides tools for creating, evaluating, and organizing scientific
hypotheses as part of a structured research workflow.
"""
...
@dataclass
class Hypothesis:
    """A scientific hypothesis with metadata for tracking and evaluation."""
    ...
    def propose(self) -> None:
        """Move the hypothesis from DRAFT to PROPOSED."""
    ...
    def start_testing(self) -> None:
        """Move the hypothesis to TESTING status."""
    ...
    def refute(self, confidence: float | None = None) -> None:
        """Mark the hypothesis as REFUTED by evidence."""
        if self.status != HypothesisStatus.TESTING:
    ...
    def revise(self, new_statement: str, new_rationale: str | None = None) -> None:
        """Revise the hypothesis, resetting it to REVISED status."""
...
class HypothesisStore:
    """In-memory store for managing a collection of hypotheses."""
    ...
    def remove(self, hypothesis_id: str) -> Hypothesis:
        """Remove and return a hypothesis by id."""
        try:
    ...
    def list_all(self) -> list[Hypothesis]:
        """Return all hypotheses ordered by creation time."""
